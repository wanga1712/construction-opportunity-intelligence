...
class HydroLeadRepository:
    ...
    def list_leads(self, filters: dict[str, Any] | None = None, sort: str = "work_queue", limit: int = 50, offset: int = 0) -> list[HydroLeadCardDTO]:
        ...
        if filters.get("text"): clauses.append("(coalesce(mc.name,'') || ' ' || coalesce(po.address,'') || ' ' || coalesce(po.cadastral_number,'') || ' ' || coalesce(mc.inn,'')) ILIKE %s"); params.append(f"%{filters['text']}%")
