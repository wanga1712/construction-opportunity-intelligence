"""V3 prompt v6.1 — calibration iteration after v6 A/B (SHADOW only).

Changes vs v6:
- filled linoleum/flooring + cable_support examples
- forbid inventing materials for generic buildings
- silent-empty ban restated at end
- multi-product DIRECT may emit multiple registry hits or REVIEW_REQUIRED
...
def build_v6_prompt_from_model_input(
...
def build_v6_prompt(
    ...
    *,
    ...
    if isinstance(model_input, dict) and model_input.get("model_input_version"):
        return build_v6_prompt_from_model_input(
            model_input,
            registry=registry,
