...
class LearningObserver:
        ...
        try:
            ...
            for item in queue_items:
                ...
                with crm_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as c_cur:
                    ...
                    # Fetch real canonical document links (NO fabricated specification_<pid>.pdf)
                    c_cur.execute("""
                        SELECT id, document_name, source_url, extension
                        FROM crm_procurement_documents
                        WHERE procurement_id = %s
                    """, (pid,))
                    doc_rows = c_cur.fetchall()

                    document_manifest = []
                    for idx, d in enumerate(doc_rows, start=1):
                        document_manifest.append({
                            "source_document_id": d["id"],
                            "document_key": f"doc_{pid}_{d['id']}",
                            "document_name": d.get("document_name") or f"document_{d['id']}",
                            "source_url": d.get("source_url"),
                            "extension": d.get("extension") or "pdf"
                        })
    ...
    def _build_missing_truths(self) -> int:
        ...
        try:
            with doc_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                # TRUTH ONLY FOR TERMINAL EXHAUSTIVE RESEARCH ITEMS
                cur.execute("""
            ...
            for item in term_items:
                ...
                with crm_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as c_cur:
                    ...
                    # Query generation-scoped raw evidence
                    c_cur.execute("""
                    ...
                    if ev_cnt == 0:
                        # Fallback for current S13_V2 rows
        ...
        try:
            with crm_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                for pair in pairs:
                    ...
                    # Real evaluation metrics: NULL if 0 useful docs
                    doc_recall_1 = None
                    doc_recall_3 = None
                    doc_recall_5 = None
                    mrr = None
