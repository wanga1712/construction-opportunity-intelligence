...
def _load_sync_info() -> dict:
    try:
        ...
        from psycopg2.extras import RealDictCursor
        import os
        PG = dict(host=os.environ.get("CRM_DB_HOST","S7"),
                  port=int(os.environ.get("CRM_DB_PORT",5432)),
                  user=os.environ.get("CRM_DB_USER","postgres"),
                  password=os.environ.get("CRM_DB_PASSWORD","<REMOVED_COMPROMISED_CREDENTIAL>"),
                  dbname="crm")
...
def _render_torgi_card(card: dict, idx: int) -> None:
    ...
    with st.container(border=True):
        ...
        with right:
            ...
            if st.button("↻", key=f"torgi_reenrich_{idx}", use_container_width=True,
                         help="Повторно обогатить закупку"):
                from src.services.crm_profile_service import queue_reenrich
                ok = queue_reenrich(card.get("id", 0))
                st.toast("Добавлено в очередь переобогащения" if ok else "Уже в очереди")
