...
def create_processing_backend(backend_name: str, db=None) -> ProcessingBackend:
    """
    Return ProcessingBackend for S13_V2 or S13_V4, or LEGACY fallback.
    Raises RuntimeError if selected S13 backend and DB unreachable.
    """
    if backend_name in ("S13_V2", "S13_V4"):
        dsn = _load_s13_dsn()
        gen = "S13_V4_EXHAUSTIVE_CONTEXT" if backend_name == "S13_V4" else "S13_V2"
        queue_repo = S13V2QueueRepository(dsn, pipeline_generation=gen)
        
        from .s13_results import S13V2ResultStore
        result_store = S13V2ResultStore(dsn, pipeline_generation=gen)
        state_repo = S13V2StateRepository(dsn, pipeline_generation=gen)
