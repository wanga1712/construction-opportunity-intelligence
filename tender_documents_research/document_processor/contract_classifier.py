...
def _call_ollama(prompt: str, timeout: int = 45) -> Optional[Dict[str, float]]:
    # CRM-V3 shared GPU arbiter: one heavy inference at a time; docs defer to routing.
    try:
        from document_processor.docs_gpu_arbiter_lock import acquire_document_gpu
        with acquire_document_gpu(defer_when_routing=True):
            return _call_ollama_unlocked(prompt, timeout=timeout)
    except Exception:
        return _call_ollama_unlocked(prompt, timeout=timeout)


def _call_ollama_unlocked(prompt: str, timeout: int = 45) -> Optional[Dict[str, float]]:
    body = json.dumps({
        "model": OLLAMA_MODEL,
        "prompt": prompt,
        "stream": False,
