...
2. **Сервер `<S7_SSH_USER>`**:
   - не удалось получить runtime-данные с хоста `<S7_SSH_USER>` по SSH (hostname не разрешился в текущем окружении);
   - alias `S7` из SSH-конфига также недоступен по сети из этой сессии.
