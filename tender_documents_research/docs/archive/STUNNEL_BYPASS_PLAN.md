...
## 📋 Файлы для копирования на сервер S7

```bash
scp improved_http_client.py S7:/opt/tendermonitor/
scp patch_http_client.py S7:/opt/tendermonitor/
scp patch_downloader.py S7:/opt/tendermonitor/
scp deploy_stunnel_bypass.sh S7:/opt/tendermonitor/
scp check_system_status.py S7:/opt/tendermonitor/
scp monitor_recovery.py S7:/opt/tendermonitor/
```

## 🔧 Команды на сервере S7
