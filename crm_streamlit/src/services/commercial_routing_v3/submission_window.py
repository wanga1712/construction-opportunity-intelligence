...
def remaining_submission_days(
    ...
    *,
    ...
    if isinstance(deadline, date):
        return float((deadline - today).days)
    text = str(deadline).strip()
    try:
        if "T" in text or " " in text:
            return remaining_submission_days(
...
def is_actionable_submission_window(
    deadline: Any,
    *,
    now: datetime | None = None,
    today: date | None = None,
...
def actionable_submission_sql(alias: str = "cp", column: str = "end_date") -> str:
    return (
        f"{alias}.{column} >= CURRENT_DATE + INTERVAL "
        f"'{MIN_REMAINING_SUBMISSION_DAYS} days'"
    )
