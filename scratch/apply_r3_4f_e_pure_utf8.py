...
# -*- coding: utf-8 -*-
"""
Applies R3-4F-E changes cleanly using Python's native UTF-8 file IO.
"""
import os
...
print("Updated context_validator.py in pure UTF-8.")
...
assert old_trusted_blocks in svc_text, "Could not find v3_trusted block in context_validator_service.py"
svc_text = svc_text.replace(old_trusted_blocks, new_trusted_blocks, 1)
...
assert old_precedence in svc_text, "Could not find v3_trusted precedence block in context_validator_service.py"
svc_text = svc_text.replace(old_precedence, new_precedence, 1)

with open(SVC_PATH, "wb") as f:
    f.write(svc_text.encode("utf-8"))

print("Updated context_validator_service.py in pure UTF-8.")

# 3. Create tests/test_context_validator_v4_decision_boundary.py
...
with open(V4_TEST_PATH, "wb") as f:
    f.write(v4_test_code.encode("utf-8"))

print("Created tests/test_context_validator_v4_decision_boundary.py in pure UTF-8.")
