...
pid = 160646
header = fetch_procurement_header(crm_db, pid)
...
raw_ev = load_current_generation_raw_evidence(pid, crm_db)
...
doc_proof = {
    ...
    "ALL_DOCUMENTS_VISIBLE": len(card_view["documents"]) > 0,
    ...
    "SAMPLE_DOCUMENTS": [
        {
            ...
            "raw_snippets": [ev.get("raw_text") for ev in (d.get("research_evidence") or [])],
        }
        for d in card_view["documents"] if d.get("research_evidence")
    ]
}
