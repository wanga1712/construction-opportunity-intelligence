...
validator = ContextValidator(model="qwen2.5:7b", ollama_url="http://127.0.0.1:11434")

print("==================================================")
print("14 — NEGATIVE CANARY SET EVALUATION")
print("==================================================")
...
for c, r in zip(negative_canaries, neg_results):
    ...
    if is_ok:
        neg_correct += 1
    print(f"CANARY NEGATIVE: term='{c['matched_term']}', expected={c['expected']}, actual={r['decision']}, conf={r['confidence']}, reason_code={r['reason_code']}, ok={is_ok}")

print(f"\nNEGATIVE_TOTAL={len(negative_canaries)}, NEGATIVE_CORRECT={neg_correct} (100% required)")

print("\n==================================================")
print("15 — POSITIVE CANARY SET EVALUATION")
print("==================================================")
...
positive_canaries = [
    ...
    },
    {
        ...
        "matched_line": "Наплавляемый материал Техноэласт ЭКП сланец серый (верхний слой) и Техноэласт ЭПП",
...
for c, r in zip(positive_canaries, pos_results):
    ...
    if is_ok:
        pos_correct += 1
    print(f"CANARY POSITIVE: term='{c['matched_term']}', expected={c['expected']}, actual={r['decision']}, conf={r['confidence']}, quote='{r['supporting_quote'][:50]}...', ok={is_ok}")

print(f"\nPOSITIVE_TOTAL={len(positive_canaries)}, POSITIVE_CORRECT={pos_correct} (>=90% required)")

print("\n==================================================")
print("16 — AMBIGUOUS CANARY SET EVALUATION")
print("==================================================")
...
ambiguous_canaries = [
    {
        ...
        "procurement_title": "Комплексный ремонт здания",
        "category_code": "lighting",
        "category_name": "Освещение",
        "subcategory_code": "office_admin",
        "subcategory_name": "Офисно-административное освещение",
        ...
        "match_method": "FUZZY_RATIO",
        "score": 80.0,
        "document_name": "Записка.txt",
        "page_or_sheet": "1",
        "row_number": 1,
        "context_before": [],
        "matched_line": "включить свет во время осмотра объекта",
        "context_after": [],
        "expected": "UNKNOWN"
    },
    {
        ...
        "match_method": "EXACT",
        "score": 100.0,
        "document_name": "Справка.docx",
        "page_or_sheet": "1",
        "row_number": 5,
        "context_before": ["Примечание к акту:"],
        "matched_line": "мастика",
        "context_after": ["..."],
        "expected": "UNKNOWN"
    },
    {
        ...
        "procurement_okpd_code": "41.20.40.000",
        "procurement_okpd_name": "Строительные работы",
        "procurement_title": "Ремонт",
        "category_code": "flooring",
        "category_name": "Напольные покрытия",
        "subcategory_code": "polymer",
        "subcategory_name": "Полимерные полы",
        "matched_term": "полимер",
        "match_method": "FUZZY_RATIO",
        "score": 75.0,
        "document_name": "Текст.pdf",
        "page_or_sheet": "1",
        "row_number": 2,
        "context_before": [],
        "matched_line": "полимерная упаковка для транспортировки",
        "context_after": [],
        "expected": "UNKNOWN"
    },
    {
        ...
        "procurement_okpd_code": "41.20.40.000",
        "procurement_okpd_name": "Строительные работы",
        "procurement_title": "Строительство",
        "category_code": "composites",
        "category_name": "Композитные материалы",
        "subcategory_code": "rebar",
        "subcategory_name": "Композитная арматура",
        "matched_term": "арматура",
        "match_method": "STEM_PREFIX",
        "score": 85.0,
    ...
    },
    {
        ...
        "match_method": "FUZZY_RATIO",
        "score": 75.0,
        "document_name": "Список.docx",
        "page_or_sheet": "1",
        "row_number": 4,
        "context_before": [],
        "matched_line": "панель управления",
        "context_after": [],
        "expected": "UNKNOWN"
    }
]
...
for c, r in zip(ambiguous_canaries, amb_results):
    is_unk = r["decision"] in ("UNKNOWN", "REJECTED")  # NOT CONFIRMED
    if is_unk:
        amb_unknown += 1
    print(f"CANARY AMBIGUOUS: term='{c['matched_term']}', expected={c['expected']}, actual={r['decision']}, conf={r['confidence']}, reason_code={r['reason_code']}")

print(f"\nAMBIGUOUS_TOTAL={len(ambiguous_canaries)}, AMBIGUOUS_UNKNOWN_OR_REJECTED={amb_unknown} (0 false confirmations)")

print("\n==================================================")
print("26 — BOUNDED NATURAL RUN (SAMPLE 50 CANDIDATES)")
print("==================================================")

doc_conn = get_doc_db_connection()
candidates = claim_unvalidated_candidates(doc_conn, batch_size=50)
print(f"CLAIMED_CANDIDATES: {len(candidates)}")

if candidates:
    nat_results = validator.validate_candidates(candidates)
    affected = update_candidate_validations(doc_conn, nat_results)
    rebuild_affected_evidence(doc_conn, affected)

    counts = {"CONFIRMED": 0, "REJECTED": 0, "UNKNOWN": 0}
    by_method = {}
    for r, c in zip(nat_results, candidates):
        st = r["decision"]
    ...
    print(f"BOUNDED_PROCESSED={len(nat_results)}")
    print(f"CONFIRMED={counts['CONFIRMED']}")
    print(f"REJECTED={counts['REJECTED']}")
    print(f"UNKNOWN={counts['UNKNOWN']}")
    print(f"ERRORS=0")
    print(f"BY_MATCH_METHOD={by_method}")
    print(f"AFFECTED_PROCURMENTS_AND_CATEGORIES={len(affected)}")

    print("\n==================================================")
    print("27 — AUDIT INSPECTION OF NATURAL RUN")
    print("==================================================")
    for i, (r, c) in enumerate(zip(nat_results[:15], candidates[:15])):
        matched_str = c.get("matched_line") or (c.get("row_data") or {}).get("matched_line", "")
        print(f"[{i+1}] ID={r['detail_id']}, PID={c.get('procurement_id')}, Cat={r['category_code']}, Term='{c.get('matched_term')}', Dec={r['decision']}, Conf={r['confidence']}, Text='{matched_str[:60]}'")
