#!/usr/bin/env python3
"""
Installer script using 100% ASCII byte representations for Russian texts.
Guarantees clean UTF-8 output on Windows without any OS encoding dependency.
"""
import os
...
# Reset files from git HEAD
os.system(f"git checkout -- {VAL_PATH} {SVC_PATH} {V2_TEST_PATH} {V3_TEST_PATH}")

# 1. Prepare SYSTEM_PROMPT and question_block as raw UTF-8 bytes
sys_prompt_bytes = """SYSTEM_PROMPT = \"\"\"Ты — эксперт-валидатор совпадений в документах госзакупок для CRM строительных материалов, оборудования и работ.
...
q_block_bytes = """        # Bounded Question Block
        question_block = (
            f"\\n[ВОПРОС]\\n"
...
# Update context_validator.py via bytes
...
assert sys_start != -1 and sys_end != -1, "Could not find SYSTEM_PROMPT bytes"
...
assert q_start != -1 and q_end != -1, "Could not find question_block bytes"
val_data = val_data[:q_start] + q_block_bytes + b"\n\n        " + val_data[q_end:]

with open(VAL_PATH, "wb") as f:
    f.write(val_data)

print("1. context_validator.py updated in pure UTF-8 bytes.")

# 2. Update context_validator_service.py via bytes
...
assert old_trusted in svc_data, "Could not find v3_trusted block in context_validator_service.py"
...
assert old_prec in svc_data, "Could not find v3_trusted precedence in context_validator_service.py"
svc_data = svc_data.replace(old_prec, new_prec, 1)

with open(SVC_PATH, "wb") as f:
    f.write(svc_data)

print("2. context_validator_service.py updated in pure UTF-8 bytes.")
...
print("5. Created tests/test_context_validator_v4_decision_boundary.py in pure UTF-8 bytes.")
