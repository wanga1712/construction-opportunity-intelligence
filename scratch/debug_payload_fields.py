...
c = enriched[0]
print("DEBUG CANDIDATE 36344:")
for k, v in c.items():
    print(f"  {k}: type={type(v)} len={len(str(v))} val={str(v)[:80]}")

validator = ContextValidator()
c_hyd = hydrate_candidate_context(c)
print("\nAFTER HYDRATION:")
for k in ["matched_line", "context_before", "context_after"]:
    print(f"  {k}: type={type(c_hyd.get(k))} len={len(str(c_hyd.get(k)))}")
