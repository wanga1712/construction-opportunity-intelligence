...
with doc_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur2:
    cur2.execute("SELECT COUNT(*) as cnt FROM document_match_details WHERE validator_version = 'v4' AND validated_at IS NOT NULL")
    v4_cnt = cur2.fetchone()["cnt"]
cur2.close()

doc_conn.close()
print("Total Validated V4 Rows in DB Now:", v4_cnt)
