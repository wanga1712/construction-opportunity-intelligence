...
replacement = '''class S13V2StateRepository(ProcessingStateRepository):
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE procurement_id = %s OR source_id = %s
    ...
    def _ensure_file_exists(self, procurement_id: int, table_source: str, file_name: str, url_hash: Optional[str] = None) -> Tuple[int, str, int, int]:
        """Returns (file_id, actual_gen, queue_id, crm_pid)."""
        import hashlib
        if not url_hash:
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT id, procurement_id, source_id, pipeline_generation FROM document_processing_queue
                    WHERE procurement_id = %s OR source_id = %s
                    ORDER BY (status = 'PROCESSING') DESC, id DESC
                    LIMIT 1
                """, (procurement_id, procurement_id))
                ...
                source_id = q_row[2] if q_row else None
                actual_gen = q_row[3] if q_row else self.pipeline_generation
                ...
                row = cur.fetchone()
                if row:
                    return row[0], actual_gen, queue_id, crm_pid
                ...
                cur.execute(sql, (
                    queue_id, crm_pid, table_source, source_id,
                    url, url_hash, file_name, actual_gen
                ))
                new_id = cur.fetchone()[0]
            conn.commit()
            return new_id, actual_gen, queue_id, crm_pid
            ...
            return 0, self.pipeline_generation, None, procurement_id
    ...
    def mark_file_status(self, procurement_id: int, table_source: str, file_name: str, url_hash: str, status: str, worker_id: int = None):
        ...
        file_id, actual_gen, queue_id, crm_pid = self._ensure_file_exists(procurement_id, table_source, file_name, url_hash)
            ...
            try:
                ...
                with conn.cursor() as cur:
                    cur.execute("""
                        ...
                        WHERE file_id = %s AND pipeline_generation = %s
                        LIMIT 1
                    """, (file_id, actual_gen))
                    r_row = cur.fetchone()
                    ...
                    if r_row:
                        cur.execute("""
                            ...
                            SET status = %s,
                                ...
                                worker_id = COALESCE(%s, worker_id)
                            WHERE id = %s
                        """, (status_val, error_message, worker_id, r_row[0]))
                    else:
                        cur.execute("""
                            INSERT INTO document_processing_results (
                                queue_id, file_id, procurement_id, status, worker_id,
                                pipeline_generation, created_at, completed_at, error_message
                            ) VALUES (
                                %s, %s, %s, %s, %s,
                                %s, NOW(), NOW(), %s
                            )
                        """, (queue_id, file_id, crm_pid, status_val, worker_id, actual_gen, error_message))
                conn.commit()
            except Exception as dpr_exc:
                try:
                    self._get_conn().rollback()
                except Exception:
                    pass
                self.logger.error(f"Error persisting document_processing_results: {dpr_exc}")
