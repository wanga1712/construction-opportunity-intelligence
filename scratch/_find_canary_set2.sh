...
old_ai_skip_44 = []
for r in p44:
    res = resolve_document_links(source_table=r["source_table"], source_id=r["source_id"], contract_number=r["contract_number"])
    links = res.get("links") or []
    if links:
        valid_44.append((r["id"], r["source_table"], r["ai_action"], len(links)))
        if r["ai_action"] in ("SKIP", "METADATA_ONLY", None):
            old_ai_skip_44.append(r["id"])

print(f"Found {len(valid_44)} 44-ФЗ procurements with documents. Sample: {valid_44[:5]}")
print(f"Old AI skip candidates 44: {old_ai_skip_44[:5]}")
...
for r in p223:
    ...
    links = res.get("links") or []
    if links:
        valid_223.append((r["id"], r["source_table"], r["ai_action"], len(links)))
        if r["ai_action"] in ("SKIP", "METADATA_ONLY", None):
            old_ai_skip_223.append(r["id"])

print(f"Found {len(valid_223)} 223-ФЗ procurements with documents. Sample: {valid_223[:5]}")
print(f"Old AI skip candidates 223: {old_ai_skip_223[:5]}")
