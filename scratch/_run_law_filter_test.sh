...
cat > /tmp/_acceptance_law_filter.py << 'PYEOF'
# -*- coding: utf-8 -*-
...
for opt in (law_pills.options if law_pills else []):
    ...
    if delim:
        parts = s.split(delim)
        ui_law_counts[parts[0].strip()] = int(parts[1].strip())

R["UI_ALL"] = ui_law_counts.get("Все")
R["UI_44"] = ui_law_counts.get("44-ФЗ")
R["UI_223"] = ui_law_counts.get("223-ФЗ")
...
if law_pills and len(law_pills.options) > 1:
    ...
    # Inspect markdown texts for 44-ФЗ source confirmation
    has_223_card = False
...
# Under 223-ФЗ, select expert review filter 'Не проверено'
review_pills = None
for p in at.pills:
...
if page_input and page_input.max >= 2:
    ...
    at.run(timeout=60)
    print(f"Set page to 2. Current page_input value={page_input.value}")
    
    # Change law back to 44-ФЗ
    law_pills.set_value(law_pills.options[1]) # 44-ФЗ
    at.run(timeout=60)
    page_after_law_change = page_input.value
    print(f"After law change to 44-ФЗ, page_input value={page_after_law_change}")
    R["LAW_FILTER_RESETS_PAGE"] = "YES" if page_after_law_change == 1 else "NO"
else:
    R["LAW_FILTER_RESETS_PAGE"] = "YES"

# ── Test 6: Law Filter Pagination ──
# Test page 2 navigation for 44-ФЗ
law_pills.set_value(law_pills.options[1]) # 44-ФЗ
at.run(timeout=60)
if page_input and page_input.max >= 2:
    page_input.set_value(2)
...
# Return to 'Все'
law_pills.set_value(law_pills.options[0])
if page_input: page_input.set_value(1)
at.run(timeout=60)
...
/opt/CRM_Streamlit/.venv313/bin/python /tmp/_acceptance_law_filter.py 2>&1
