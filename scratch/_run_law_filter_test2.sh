...
cat > /tmp/_acceptance_law_filter2.py << 'PYEOF'
# -*- coding: utf-8 -*-
...
# Navigate to page 2 first under 'Все'
if page_input and page_input.max >= 2:
    page_input.set_value(2)
    at.run(timeout=60)
    print(f"Set page to 2 under ALL. current value={page_input.value}")
...
# Under 44-ФЗ, navigate to page 2 if available
law_pills.set_value(law_pills.options[1]) # 44-ФЗ
at.run(timeout=60)
if page_input and page_input.max >= 2:
    page_input.set_value(2)
...
/opt/CRM_Streamlit/.venv313/bin/python /tmp/_acceptance_law_filter2.py 2>&1
