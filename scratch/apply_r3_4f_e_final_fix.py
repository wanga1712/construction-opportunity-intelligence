...
# Reset context_validator.py from HEAD first
os.system(f"git checkout -- {VAL_PATH} {V2_TEST_PATH} {V3_TEST_PATH}")

# 1. Read context_validator.py in utf-8
with codecs.open(VAL_PATH, "r", encoding="utf-8") as f:
    text = f.read()
...
text = text[:s_idx] + sys_prompt_clean + "\n\n\n" + text[e_idx:]
...
text = text[:qs_idx] + q_block_clean + "\n\n        " + text[qe_idx:]

with codecs.open(VAL_PATH, "w", encoding="utf-8") as f:
    f.write(text)

print("Updated context_validator.py with clean UTF-8.")

# 2. Update test_context_validator_semantics_v2.py to use dynamic constants
with codecs.open(V2_TEST_PATH, "r", encoding="utf-8") as f:
    v2_text = f.read()
...
with codecs.open(V2_TEST_PATH, "w", encoding="utf-8") as f:
    f.write(v2_text)

print("Updated test_context_validator_semantics_v2.py.")

# 3. Update test_context_validator_v3_trust_boundary.py
with codecs.open(V3_TEST_PATH, "r", encoding="utf-8") as f:
    v3_text = f.read()

v3_text = v3_text.replace('assert VALIDATOR_VERSION == "v3"', 'assert VALIDATOR_VERSION in ("v3", "v4")')

with codecs.open(V3_TEST_PATH, "w", encoding="utf-8") as f:
    f.write(v3_text)

print("Updated test_context_validator_v3_trust_boundary.py.")

# 4. Write test_context_validator_v4_decision_boundary.py
...
with codecs.open(V4_TEST_PATH, "w", encoding="utf-8") as f:
    f.write(v4_test_code)

print("Created tests/test_context_validator_v4_decision_boundary.py.")
