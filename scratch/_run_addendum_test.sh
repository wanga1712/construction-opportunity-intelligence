...
cat > /tmp/_acceptance_addendum.py << 'PYEOF'
# -*- coding: utf-8 -*-
...
# Check pills widget
pills_widget = at.pills(key="annotation_state_filter_torgi_stage_workspace")
if not pills_widget:
    for p in at.pills:
        if any(x in str(p.label) for x in ["Экспертная", "разметка", "filter"]):
            pills_widget = p
            break

print(f"PILLS_WIDGET_LABEL={repr(pills_widget.label if pills_widget else None)}")
...
for opt in pill_options:
    ...
    # Check rendered cards and pagination caption
    page_caption = None
    for c in at.caption:
...
/opt/CRM_Streamlit/.venv313/bin/python /tmp/_acceptance_addendum.py 2>&1
