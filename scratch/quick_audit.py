...
print("Connections established.", flush=True)
...
print("Priors loaded:", len(priors), flush=True)
...
crm_total = len(rows)
print("CRM procurements count:", crm_total, flush=True)
...
print(f"Target PIDs count: {target_id_count}, payload bytes: {payload_bytes}", flush=True)
...
v4_total = len(details)
print("Unvalidated V4 details count:", v4_total, flush=True)

v4_target = sum(1 for r in details if r["procurement_id"] in target_id_set)
