...
path_disc = "/opt/CRM_Streamlit_rescue/src/services/commercial_routing_v3/evidence_discovery.py"
code_disc = '''import hashlib, json, re
...
def load_discovery_vocabulary(crm_db) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for c in cats:
            ...
            if name:
                vocab.append({"term": name.lower(), "category_code": code, "method": "CATEGORY_NAME_MATCH"})
    ...
    general_terms = [
        "поставка", "комплект", "кабель", "светильник", "фибра", "лоток",
        "гидроизоляция", "разъем", "оборудование", "материал", "арматура",
        "труба", "провод", "автомат", "клапан", "насос", "трансформатор"
    ]
...
def discover_and_persist_raw_evidence(
    ...
    for unit in iter_parsed_units(procurement_id, source_table, source_id, contract_number):
        ...
        if not matched_entry and len(text_lower) >= 5:
            matched_entry = {"term": unit.raw_text[:50], "category_code": None, "method": "PARSED_UNIT_FALLBACK"}
...
print("DEPLOYED CLEAN evidence_discovery.py TO S13!")
