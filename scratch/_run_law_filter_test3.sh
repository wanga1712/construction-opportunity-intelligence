...
cat > /tmp/_acceptance_law_filter3.py << 'PYEOF'
# -*- coding: utf-8 -*-
...
def get_page_input():
    for ni in at.number_input:
        if str(ni.key).startswith("torgi_workset_page"):
            return ni
    return None

page_input = get_page_input()
if page_input and page_input.max >= 2:
    page_input.set_value(2)
    at.run(timeout=60)
    page_input = get_page_input()
    print(f"Set page to 2 under ALL. current value={page_input.value if page_input else 'None'}")
    
    # Change law to 44-ФЗ
    law_pills.set_value(law_pills.options[1]) # 44-ФЗ
    at.run(timeout=60)
    
    page_input_after = get_page_input()
    val_after = page_input_after.value if page_input_after else 1
    print(f"After law change to 44-ФЗ, page_input key={page_input_after.key if page_input_after else 'None'} value={val_after}")
...
/opt/CRM_Streamlit/.venv313/bin/python /tmp/_acceptance_law_filter3.py 2>&1
