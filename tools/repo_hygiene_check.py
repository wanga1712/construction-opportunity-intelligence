...
def git_tracked_files() -> list[str]:
    out = subprocess.check_output(["git", "ls-files", "-z"], cwd=ROOT, text=True, errors="replace")
...
def main() -> int:
    ...
    for rel in git_tracked_files():
        ...
        for rule_name, pattern in GENERIC_RULES:
            if rule_name == "postgres_uri_with_password":
                if any(
                    pattern.search(line) and not line.lstrip().startswith("#")
                    for line in text.splitlines()
                ):
                    findings.append(f"{rel}  [{rule_name}]")
            elif pattern.search(text):
                findings.append(f"{rel}  [{rule_name}]")
        if rel.replace("\\", "/") == ".hygiene/denylist.txt":
            continue
