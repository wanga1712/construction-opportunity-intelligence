...
def main() -> int:
    ...
    findings: list[str] = []
    denylist_resolved = Path(denylist_source).resolve() if denylist_source else None
    for rel in git_tracked_files():
        if rel.startswith(".hygiene/"):
            continue
        ...
        if path_rule:
            ...
            continue
        path = ROOT / rel
        if denylist_resolved and path.resolve() == denylist_resolved:
            continue
        if not path.is_file():
            continue
        try:
