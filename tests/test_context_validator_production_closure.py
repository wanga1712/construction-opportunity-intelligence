...
def test_13_structurally_ambiguous_rejected_demoted_to_unknown():
    """13. Truncated/table fragments cannot remain REJECTED — must be UNKNOWN."""
    def reject_caller(prompt: str) -> str:
        return json.dumps({
            "detail_id": 303,
            "decision": "REJECTED",
            "confidence": 1.0,
            "supporting_quote": "пропитка",
            "reason_code": "INSUFFICIENT_CONTEXT",
            "reason": "model incorrectly rejected truncated cell",
        })
...
def test_14_only_confirmed_creates_evidence():
    """14. Positive document_evidence is inserted only for CONFIRMED match details."""
    mock_conn = MagicMock()
    mock_cur = MagicMock()
    mock_conn.cursor.return_value.__enter__.return_value = mock_cur
