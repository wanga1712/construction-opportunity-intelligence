...
class TestPriorityScore(unittest.TestCase):
    ...
    def test_ranked_gold_ranks_above_ranked_silver(self):
        """Ranked Gold (commercial>=75, stage=ranked) outranks ranked Silver."""
        gold   = _card(award_status="submission_open", commercial_score=80,
                       processing_stage="ranked", days_offset=10)
        silver = _card(award_status="submission_open", commercial_score=55,
                       processing_stage="ranked", days_offset=10)
        self.assertGreater(_torgi_priority_score(gold), _torgi_priority_score(silver))

    def test_ranked_silver_above_ranked_bronze(self):
        bronze = _card(award_status="submission_open", commercial_score=30,
                       processing_stage="ranked", days_offset=10)
        silver = _card(award_status="submission_open", commercial_score=55,
                       processing_stage="ranked", days_offset=10)
        self.assertGreater(_torgi_priority_score(silver), _torgi_priority_score(bronze))

    def test_ranked_gold_beats_ai_verified_with_evidence(self):
        """Tier 1 (ranked+score) must outrank Tier 2 (ai_verified+evidence)."""
        ranked = _card(award_status="submission_open", commercial_score=80,
                       processing_stage="ranked", days_offset=10)
        evidence_card = _card(award_status="submission_open", commercial_score=None,
                              processing_stage="ai_verified", days_offset=10)
        evidence_card["_proc"] = {"evidence_count": 500, "interesting_count": 50}
        self.assertGreater(_torgi_priority_score(ranked), _torgi_priority_score(evidence_card))

    def test_ai_verified_with_evidence_beats_standard(self):
        """Tier 2 (ai_verified+evidence) ranks above Tier 3 (standard no evidence)."""
        evidence_card = _card(award_status="submission_open", commercial_score=None,
                              processing_stage="ai_verified", days_offset=10)
        evidence_card["_proc"] = {"evidence_count": 50, "interesting_count": 5}
        standard = _card(award_status="submission_open", commercial_score=None,
                         processing_stage="matches_found", days_offset=10, signal_score=99)
        self.assertGreater(_torgi_priority_score(evidence_card), _torgi_priority_score(standard))

    def test_commercial_score_alone_without_ranked_stage_stays_tier3(self):
        """commercial_score without ranked/manager_confirmed stage → Tier 3, not Tier 1."""
        unranked_scored = _card(award_status="submission_open", commercial_score=80,
                                processing_stage="matches_found", days_offset=10)
        standard = _card(award_status="submission_open", commercial_score=None,
                         processing_stage="matches_found", days_offset=10, signal_score=0)
        # Both in Tier 3; scored card has no ranked stage so no Tier 1 bonus
        self.assertLess(_torgi_priority_score(unranked_scored), 20_000)
