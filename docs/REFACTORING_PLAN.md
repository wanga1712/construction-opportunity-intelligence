...
**CRM-V3-PRE-RESEARCH-SCOPE-CLASSIFICATION-AND-LIVE-ADMISSION-MATERIALIZATION-1** — `[~]` **PARTIAL_PASS / PRODUCTION NOT MATERIALIZED**. Feature commits `9daa385`, `2a3c259`, and the current corrective changes are on the canonical feature branch; required README/model-priority documentation is committed. Audited the existing procurement scope logic and kept one canonical classifier path, corrected it to use CRM `auction_name`, removed OKPD-only direct-goods fallback, added reproducible evidence/policy versioning, and added procurement-grain admission policy plus an explicit dry-run materializer and CRM authority DDL. Corrected read-only S13 snapshot for `CURRENT_CRM_AND_OKPD=9720`: `DIRECT_GOODS=3168`, `WORKS_WITH_EMBEDDED_PRODUCTS=2874`, `DESIGN_PROJECT=123`, `EQUIPMENT_AND_INSTALLATION=51`, `SERVICE_WITH_CONSUMABLES=1`, `PURE_SERVICE=120`, `MIXED=25`, `UNKNOWN=3358`; admission `ELIGIBLE=5112`, `EXCLUDED=1250`, `HOLD=3358`; canonical resource terms are `PRIMARY_RESEARCH_SUPPRESSED_NOW=4608`, `PERMANENTLY_EXCLUDED_NOW=1250`, `HOLD_PENDING_SCOPE=3358`, `CLASSIFIED_SCOPE_TOTAL=6362`, `CLASSIFIER_COVERAGE_PCT=65.45`. Stage totals are `OPEN=5682`, `AWARDED=4038`, `TOTAL=9720`. `POST_RESEARCH_FEATURE_COUNT=0`. Regression `0104200002926000007` is locally `AWARDED + DIRECT_GOODS + EXCLUDED`. Read-only title audit found `MISSING_TITLE=0`; UNKNOWN reasons are `INSUFFICIENT_SIGNAL=2346`, `UNSUPPORTED_PATTERN=990`, `AMBIGUOUS=22`, `MISSING_TITLE=0`. A bounded independent pre-research review inspected 146 rows, including 30 awarded direct-goods, 20 open direct-goods, 20 works, 10 design, 10 equipment, 10 pure-service, 25 mixed, 1 service-with-consumables and 20 unknown; no awarded direct-goods false exclusion was found in the reviewed sample, but this does not replace the required formal review artifact. No production DDL/data materialization, queue mutation, document download, worker release, or model retraining performed. New assertions and compileall pass; legacy test remains blocked by incomplete local sklearn runtime although `requirements-learning.txt` declares `scikit-learn`. Live queue wiring and authority materialization remain incomplete; `READY_FOR_V4_WORKER_RESTORE=NO`.
