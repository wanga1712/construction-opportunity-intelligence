...
**CRM-V3-EXPERT-REVIEW-COUNTERS-AND-DEADLINE-SORT-CORRECTION-1** — `[x]` **PASS / STOP**. Exact canonical baseline `c336678839bcb9318a0851fca90d8f9928884a84`, corresponding to prior standalone S13 runtime `6ea43cc3e36e91418a7bf70aefaa2d5195844245`. Read-only forensics prove root cause C: production contains only one operator annotation, not approximately 20 persisted reviews; there are six total current version-1 rows, five acceptance-smoke rows outside the actionable workset, and no historical versions. One persisted batch projection now reports `Все 4037 / Не проверено 4036 / Проверено 1 / Неинтересные 1`, with NOT_INTERESTING correctly a REVIEWED subset. All active save paths commit through `save_expert_annotation` before GO_NEXT/rerun, and the uncached batch projection refreshes afterward. TORGI defaults to global deadline DESC and offers ASC; both are SQL-before-pagination, NULLS LAST, price/id stable, and compose with expert filtering. Isolated/deployed S13 suites: 76 PASS each. Real route AppTest and browser acceptance PASS with both 25-date lists monotonic, procurement 11235 showing both reviewed/negative chips, zero exceptions, no production save. Implementation `ad02eb2b`; report: `docs/reports/expert_review_counters_deadline_sort/IMPLEMENTATION_AND_PRODUCTION_ACCEPTANCE.md`. No model/prompt/input, queue/worker, expert payload/schema, minimum-window, document resolver/parser, publication, DDL or 615-ФЗ change. STOP.

**Expected result recorded before behavior change:** persisted current annotation existence alone defines REVIEWED; OUT_OF_PROFILE/NCE/NO_COMMERCIAL_ENTRY remains reviewed and also NOT_INTERESTING; ALL=UNREVIEWED+REVIEWED and NOT_INTERESTING⊆REVIEWED; all save paths commit through `save_expert_annotation` before optional navigation and rerun reloads the one batch state projection; fresh TORGI defaults to deadline DESC NULLS LAST, nearest mode uses ASC NULLS LAST, both with stable price/id ties in SQL before pagination; `MIN_REMAINING_SUBMISSION_DAYS=2` remains unchanged.

**Current WIP closure note:** final production audit remained `4037/4036/1/1/0` (all/unreviewed/reviewed/not-interesting/profiled); no acceptance save was made. `stage_workspace.py` is 195 lines. Pre-existing `tabs.py` (831) and stateful `annotation_card.py` (758) remain above the size guideline because decomposing their query and persistence/rerun boundaries would reopen the separately recorded Stage 2 component task; this WIP kept changes localized and verified.

**CRM-V3-EXPERT-GUIDED-TAXONOMY-SELECTORS-AND-STEPWISE-ANNOTATION-1** — `[x]` **PASS / STOP**. Exact baseline `c5c2dc6706a01fcdae8698d5d092b35fd69140be`; no older remote tree substituted. YES now opens one shared selector-first draft directly on the card: 14 active registry categories, 101 active registry subcategories batch-loaded through factual `category_id`, human-only object/subtype/stage vocabulary, explicit normalized proposal fallback and Russian human medal choices. AI=UNASSESSED works independently; model values stay read-only until explicit acceptance. Human vocabulary is honestly `0/0/0` and approved proposals absent; fixture proves the known-value no-typing payload. First negative annotation 11235 remains `OUT_OF_PROFILE/NCE`. Isolated/deployed S13 suites 64 PASS; real route AppTest and screenshots on positive control 64132 PASS with zero exceptions; no Save/fake annotation; service active / HTTP 200. Implementation `294619fd`; report: `docs/reports/expert_guided_taxonomy_selectors/IMPLEMENTATION_AND_PRODUCTION_ACCEPTANCE.md`. No model/prompt/input, queue/worker, publication, document pipeline, registry content, schema/DDL or 615-ФЗ change. STOP.

**Expected result recorded before behavior change:** AI=UNASSESSED still exposes canonical category selection after YES; known category/subcategory/object/subtype/stage values require selections only, with no taxonomy typing; new values require an explicit secondary proposal flow with whitespace/case duplicate prevention; model values stay read-only until explicit expert acceptance; one shared opportunity draft feeds the existing payload; NO and the first production negative annotation remain unchanged.

**Current WIP closure note:** the extracted `guided_annotation.py` is 287 lines. The pre-existing stateful `annotation_card.py` is 764 lines after deleting the superseded free-text/rank renderers; it remains the shared Streamlit persistence/rerun boundary. Broader decomposition remains assigned to the existing Stage 2 component task.
...
**CRM-V3-DURABLE-AI-INFERENCE-JOB-QUEUE-1** — `[x]` **PASS / STOP**. From forensic base `b2fe151e`, inference-job-only DDL creates `crm_v3_inference_jobs` with database-enforced active identity, bounded `SKIP LOCKED` claims, heartbeat/30-minute stale recovery, retry lineage, canonical input SHA-256 and existing-runner delegation. Migration is repeatable-safe; runtime grants and targeted indexes verified. Local/S13 focused suites 6 PASS; exact standalone runtime `4203223b`, service active / HTTP 200 / tracked-clean. Candidate inference remained disabled and queue depth remained zero; no enqueue or inference executed. Implementation `f9d600f`. Report: `docs/reports/durable_ai_inference_job_queue/IMPLEMENTATION_AND_PRODUCTION_ACCEPTANCE.md`. STOP; next WIP is `CRM-V3-AI-EXPERT-REVIEW-QUEUE-AND-MANUAL-REASSESSMENT-2`.
