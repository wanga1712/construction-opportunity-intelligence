...
**CRM-V3-PRE-RESEARCH-SCOPE-CLASSIFICATION-AND-LIVE-ADMISSION-MATERIALIZATION-1** — `[~]` **PARTIAL_PASS / PRODUCTION NOT MATERIALIZED**. Feature commits `9daa385` and corrective `2a3c259` are pushed to the canonical feature branch; required README/model-priority documentation is committed. Audited the existing procurement scope logic and kept one canonical classifier path, corrected it to use CRM `auction_name`, removed OKPD-only direct-goods fallback, and added procurement-grain admission policy plus an explicit dry-run materializer and CRM authority DDL. Read-only S13 snapshot classified `CURRENT_CRM_AND_OKPD=9720` as `DIRECT_GOODS=3524`, `WORKS_WITH_EMBEDDED_PRODUCTS=1420`, `DESIGN_PROJECT=19`, `EQUIPMENT_AND_INSTALLATION=58`, `SERVICE_WITH_CONSUMABLES=3`, `PURE_SERVICE=84`, `MIXED=3`, `UNKNOWN=4609`; admission `ELIGIBLE=3840`, `EXCLUDED=1271`, `HOLD=4609`; canonical resource terms are `PRIMARY_RESEARCH_SUPPRESSED_NOW=5880`, `PERMANENTLY_EXCLUDED_NOW=1271`, `HOLD_PENDING_SCOPE=4609`, `CLASSIFIED_SCOPE_TOTAL=5111`, `CLASSIFIER_COVERAGE_PCT=52.58`. `POST_RESEARCH_FEATURE_COUNT=0`. Regression `0104200002926000007` is locally `AWARDED + DIRECT_GOODS + EXCLUDED`. Read-only title audit found `MISSING_TITLE=0`; UNKNOWN is insufficient recognized scope signal, not absent title. No production DDL/data materialization, queue mutation, document download, worker release, or model retraining performed. New assertions and compileall pass; legacy test is blocked by incomplete local sklearn runtime although `requirements-learning.txt` declares `scikit-learn`. Minimum independent quality review and live queue wiring remain incomplete; `READY_FOR_V4_WORKER_RESTORE=NO`.
