...
**CRM-V3-RESEARCH-PRIOR-V2-REAL-DWRR-CLAIM-CORRECTION-1** — `[x]` **PASS / STOP**. Scope: `REAL_DWRR_CLAIM_CORRECTION`. Found that production claim paths used SQL score sorting (`ORDER BY effective_score DESC`), not actual weighted bounded scheduling — WOOD items were never claimed while GOLD candidates existed. Corrected with: (1) Weighted Virtual Time (Stride) scheduler replacing deficit-counter DWRR (`virtual_time += 1/weight`, pick `min(virtual_time)`, stateful across batch_size=1 calls); (2) per-band UNION ALL pool query ensuring candidate diversity across all bands; (3) two-phase claim (SQL FOR UPDATE SKIP LOCKED → Python Stride select → SQL claim selected IDs → COMMIT). Created shared `DWRRClaimPolicy` module. Corrected terminology: actual algorithm is Stride/Weighted Virtual Time, not classical DWRR (class name retained as backward-compat alias). Tests: 33/33 PASS. Live S13 proof N=100 sequential batch_size=1: GOLD=45 (45.0%), SILVER=28 (28.0%), BRONZE=18 (18.0%), WOOD=9 (9.0%), MAX_CONSECUTIVE_NON_WOOD=10. WOOD_CLAIMED > 0 confirmed. `ACTIVE_PRODUCTION_S13_VERIFIED`. Queue status unchanged (32625 PRE_RESEARCH_WAITING, 40 PROCESSING, 111 COMPLETED, 100 FAILED, 549 NO_LINKS). Model artifact SHA256 `d0c8e87995e9ee1545b42087352a87ba3d1aa18de1f6d9e7b4e892c6a8a5d88b` unchanged. `MODEL_CONTROLS_ADMISSION=NO`. STOP after WIP.
