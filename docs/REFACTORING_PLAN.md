...
**CRM-V3-PRE-RESEARCH-SCOPE-CLASSIFICATION-AND-LIVE-ADMISSION-MATERIALIZATION-1** — `[~]` **LOCAL PREPARATION / PRODUCTION NOT MATERIALIZED**. Audited the existing procurement scope logic and kept one canonical classifier path, corrected it to use CRM `auction_name`, removed OKPD-only direct-goods fallback, and added procurement-grain admission policy plus an explicit dry-run materializer and CRM authority DDL. Read-only S13 snapshot classified `CURRENT_CRM_AND_OKPD=9720` as `DIRECT_GOODS=3524`, `WORKS_WITH_EMBEDDED_PRODUCTS=1420`, `DESIGN_PROJECT=19`, `EQUIPMENT_AND_INSTALLATION=58`, `SERVICE_WITH_CONSUMABLES=3`, `PURE_SERVICE=84`, `MIXED=3`, `UNKNOWN=4609`; admission `ELIGIBLE=3840`, `EXCLUDED=1271`, `HOLD=4609`; `POST_RESEARCH_FEATURE_COUNT=0`. Regression `0104200002926000007` is locally `AWARDED + DIRECT_GOODS + EXCLUDED`. No production DDL/data materialization, queue mutation, document download, worker release, or model retraining performed. Targeted scope/admission assertions and compileall pass; legacy classifier test is blocked by missing `sklearn` in the local bundled runtime. Required bounded human quality sample and live queue wiring remain outstanding; `READY_FOR_V4_WORKER_RESTORE=NO`.
