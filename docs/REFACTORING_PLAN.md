...
**CRM-V3-ANALYTICS-WORKSET-AND-CARD-PRESENTATION-CORRECTION-1** — `[~]` **IMPLEMENTED / PRE-DEPLOY VERIFIED**. Analytics expert workset is separated from unchanged manager publication authority. Current 2026-08-23 waterfall: lifecycle-valid torgi 6371, manager-visible 20; hidden 6302 UNASSESSED, 7 SCOPE_UNKNOWN, 42 NO_VISIBLE_OPPORTUNITY. True commission/awarded totals are 31357/5691; all stages load bounded 25-card pages. Cards use compact title/facts/chips, factual source action above lazy pills navigation, full dates and no raw technical status line. Isolated S13 suite 73 PASS; real `app.py` route PASS with resolver `0→1`, 25 cards retained and zero exceptions. Deployment/closure pending.
