...
**CRM-V3-PRE-RESEARCH-SCOPE-CLASSIFICATION-AND-LIVE-ADMISSION-MATERIALIZATION-1** — `[~]` **PARTIAL_PASS / PRODUCTION NOT MATERIALIZED**. Feature commits `9daa385`, `2a3c259`, and `71e90d9` are on the canonical feature branch. Audited the existing procurement scope logic and kept one canonical classifier path, corrected it to use CRM `auction_name`, removed OKPD-only direct-goods fallback, added reproducible evidence/policy versioning, and added procurement-grain admission policy plus an explicit dry-run materializer and CRM authority DDL. Frozen read-only snapshot `cfbcadbb8c741526e6563e8e2825ff9230952f56c2ee65625abf89803ac237ca` contains `CURRENT_CRM_AND_OKPD=9720` unique procurements. OLD_HEAD `3efca58` and CURRENT_HEAD `71e90d9` replay identically on all 9720 rows: `DIRECT_GOODS=3168`, `WORKS_WITH_EMBEDDED_PRODUCTS=2874`, `DESIGN_PROJECT=123`, `EQUIPMENT_AND_INSTALLATION=51`, `SERVICE_WITH_CONSUMABLES=1`, `PURE_SERVICE=120`, `MIXED=25`, `UNKNOWN=3358`; transition total `9720`, non-zero transitions `0`. Consequently `PREVIOUS_REPORT_REPRODUCIBLE=NO`; the prior report is stale/non-reproducible and its historical input mapping is not proven. Current admission is `ELIGIBLE=5112`, `EXCLUDED=1250`, `HOLD=3358`; resource terms are `PRIMARY_RESEARCH_SUPPRESSED_NOW=4608`, `PERMANENTLY_EXCLUDED_NOW=1250`, `HOLD_PENDING_SCOPE=3358`, `CLASSIFIED_SCOPE_TOTAL=6362`, `CLASSIFIER_COVERAGE_PCT=65.45`. Stage totals are `OPEN=5682`, `AWARDED=4038`, `TOTAL=9720`; `POST_RESEARCH_FEATURE_COUNT=0`; classifier invocations `9720`, duplicate invocations `0`. The formal row-level artifact reviews 146 rows: 127 correct, 19 underclassified UNKNOWN, false exclusions `0`, false eligible `0/146`. No production DDL/data materialization, queue mutation, document download, worker release, or model retraining performed. New assertions and compileall pass; legacy test remains blocked by unavailable local sklearn environment although `requirements-learning.txt` declares `scikit-learn`. Live queue wiring and authority materialization remain incomplete; `READY_FOR_V4_WORKER_RESTORE=NO`. Evidence: `docs/reports/pre_research_scope_replay_20260907.md`.
2026-09-07 continuation: P1A proves admission runtime paths import neither CatBoost nor sklearn. Existing S13 `.venv313` ran legacy classifier tests `20 passed`; no packages were changed. Added dependency-light shared admission policy, V4 producer authority join, fail-closed CRM bridge, and claim-path ELIGIBLE context predicate. New bounded assertions `14 PASS`, compileall and diff-check pass. Local worker package import boundary passes; actual S13 import smoke awaits deployment of this code because the current worker checkout is older. No migration, authority materialization, queue mutation, download, or worker release. Evidence: `docs/reports/pre_research_scope_queue_admission_20260907.md`.
2026-09-07 runtime correction: document DSN now uses only `S13_DOCUMENT_DB_USER/PASSWORD`; DWRR moved to the dependency-light worker package and local worker-root synthetic smoke passes. Added active admission reconciliation immediately before claim, freshness provenance requirements, and awarded lane `awarded_recent`. Read-only S13 audit found worker cwd `/opt/tender_documents_research` resolving to dirty parent Git checkout `/opt/CRM_Streamlit` at old HEAD `b83c46c`; deployment route is not proven, so no code deployment/restart, migration, authority materialization, queue mutation, download, or worker release was performed. Evidence: `docs/reports/pre_research_scope_runtime_correction_20260907.md`.
2026-09-07 dedup correction: canonical research identity is source-family plus notice number, with procurement ID fallback only when the notice is absent. Producer dedup now spans pipeline generations and preserves PROCESSING/pending, reuses successfully completed research, and retries FAILED/PARTIAL/unsuccessfully completed work on the existing identity. OPEN -> AWARDED lifecycle changes cannot create a second research job. Local bounded assertions pass; S13 deployment remains blocked because the worker checkout/deployment route is not proven.
2026-09-07 dedup correction: canonical research identity now normalizes audited registry variants to `44_FZ` or `223_FZ`; exact lifecycle table names are not identity. Lookup uses persisted canonical key with a normalized legacy-row fallback, and PostgreSQL transaction advisory locking protects check-plus-insert. `NO_LINKS` retries only after current canonical links appear; old unsuccessful COMPLETED rows are treated as PARTIAL during lookup. Both 44-FZ and 223-FZ OPEN -> AWARDED regressions pass; S13 deployment remains blocked.
2026-09-07 CRM authority bootstrap: migration 007 was preflighted and applied only to CRM through the approved DDL route. Policy V2 holds undecided scopes (`EQUIPMENT_AND_INSTALLATION`, `SERVICE_WITH_CONSUMABLES`, `PURE_SERVICE`, `MIXED`) instead of admitting or excluding them. Current exact CRM+OKPD snapshot materialized `9720` unique authority rows: `ELIGIBLE=5035`, `EXCLUDED=1130`, `HOLD_UNKNOWN=3358`, `HOLD_POLICY_UNDECIDED=197`. Read-only all-generation dedup disposition: `PROCESSING=0`, `PENDING=2956`, `COMPLETED_REUSABLE=13`, `FAILED_PARTIAL_RETRY=2`, `NO_LINKS_RETRY=20`, `NO_LINKS_DEFERRED=0`, `NEVER_RESEARCHED=2044`; `NEW_RESEARCH_REQUIRED=2066`. Completed parse/download authority is proven for `110/111`; one legacy-gap row remains ambiguous. Queue and workers were untouched.

**CRM-V3-STRUCTURED-FACT-SOURCE-AUTHORITY-AND-INDEPENDENT-ADJUDICATION-COMPLETION-1** — `[~]` **CORRECTION_REQUIRED / IN PROGRESS**. Scope: `R4C_PASS_CORRECTION`, `DESTRUCTIVE_CLEANUP_ELIMINATION`, `CANONICAL_V4_SOURCE_AUTHORITY`, `TRUE_MODEL_CALL_ACCOUNTING`, `BATCH_SCOPED_METRICS`, `INDEPENDENT_SEMANTIC_REVIEW`, `STRATIFIED_REAL_CANARY`, `FIELD_EVIDENCE_FAIL_CLOSED`, `POSITIVE_AND_NEGATIVE_LIVE_CARD_PROOF`. Created feature branch from `START_HEAD=94a8f9ed9db68119385a4f801dce2be66529919f`. Enforcing canonical V4 source text authority, true Qwen 7B model call accounting, batch-scoped precision metrics, independent semantic adjudication, stratified real canary sampling across categories/scopes, fail-closed verbatim quote verification for product/numeric fields, and positive/negative live card proof. `R4_D_START=NO`. 2026-09-07 correction: added persisted source-authority flags, migration 005 for independent semantic adjudication, migration 006 for source eligibility flags, fail-closed promotion requiring an existing `INDEPENDENT_SEMANTIC_REVIEW` record, quote metrics separated from semantic metrics, and CRM missing-entity fallback changed to `UNKNOWN`. `SOURCE_AUTHORITY_HARDCODED=NO`, `MATCH_TERM_AS_SOURCE_SNAPSHOT=0`, `AUTO_TP_FROM_NON_BLACKLIST=0`, and `AUTO_TP_FROM_QUOTE_ONLY=0` are code-level gates. Compileall and pure adjudication assertions pass; targeted pytest and live persisted-data proof remain required. `R4_C_STATUS=CORRECTION_REQUIRED`, `READY_FOR_R4_D=NO`.

2026-09-07 correction continuation: Phase A is extract-only and leaves all new runs/entities pending; Phase B writes externally supplied independent adjudications without trust mutation; Phase C alone applies trust. Added `scripts/smoke_structured_fact_canary.py` for the required one-detail DB-backed smoke. Full S13 canary remains explicitly prohibited until this smoke and targeted tests pass.

Latest verification: targeted pytest was attempted locally and blocked by missing PostgreSQL credentials; one-detail smoke reached Phase A but was blocked before selection by unavailable local `/var/run/postgresql`. Corrective commits `5b02759` and `87e5c05` are pushed to canonical origin. S13 deploy is stopped with `IDENTITY_AUTHORITY_MISSING` because the existing `s13` Git remote alias is not identified in the authoritative operating rules.
