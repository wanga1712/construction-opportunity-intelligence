...
**CRM-V3-CATEGORY-OPPORTUNITY-CARDS-AND-MULTI-MEDAL-OUTPUT-1** — `[~]` **IN PROGRESS**. Scope: `ROADMAP_RECONCILIATION`, `CATEGORY_OPPORTUNITY_AUTHORITY`, `PROCUREMENT_X_CATEGORY_READ_MODEL`, `CONFIRMED_CATEGORY_MEDALS`, `CONFIRMED_FINDINGS_SUMMARY`, `MATERIAL_POSITION_AGGREGATION`, `QUANTITY_AGGREGATION`, `POTENTIAL_SUPPLY_VALUE`, `MULTI_MEDAL_PROCUREMENT_CARDS`, `CATEGORY_MEDAL_FILTERS`, `CRM_RESULT_DRILLDOWN`. Implementation of multi-category commercial opportunity cards and read model. Represents 1 procurement -> N confirmed commercial category opportunities (e.g. Linoleum GOLD, Lighting GOLD, Curbstone WOOD) without max-medal collapse. Preserves explicit distinction between `research_prior_band` (raw model prior), `effective_service_band` (queue scheduling order), and `category_commercial_medal` (post-research confirmed commercial medal). Includes quantities by unit, material vs position counts, potential supply value methods, category/medal filters, and documentary evidence drilldown.
