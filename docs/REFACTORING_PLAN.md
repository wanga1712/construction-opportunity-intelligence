...
**CRM-V3-CATEGORY-OPPORTUNITY-AUTHORITY-CORRECTION-1** — `[~]` **IN PROGRESS**. Scope: `CATEGORY_COMMERCIAL_AUTHORITY_CORRECTION`, `STRUCTURED_FACT_JOIN`, `CATEGORY_QUANTITY_CORRECTION`, `CATEGORY_VALUE_CORRECTION`, `MULTI_MEDAL_LIVE_PROOF`, `DIRECT_GOODS_MULTI_CATEGORY_VALUE_SAFETY`. Correction of category commercial authority, connecting read model to `crm_procurement_category_opportunities` and `crm_v3_expert_annotations` (`CATEGORY_MEDAL_FROM_RESEARCH_PRIOR=NO`, `INDEPENDENT_CATEGORY_MEDALS=YES`), joining live `structured_entities` facts (`quantity_value`, `quantity_unit`, `unit_price`, `total_price`), eliminating hardcoded commercial state/authority (`COMMERCIAL_STATE_HARDCODED=NO`), and enforcing NMCK upper bound safety for multi-category direct goods (`MULTI_CATEGORY_DIRECT_NMCK_DUPLICATION=0`).
